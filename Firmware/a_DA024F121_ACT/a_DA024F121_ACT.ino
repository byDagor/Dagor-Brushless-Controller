...
//###########################################################################
// Dagor Controller Alpha 2.4 Firmware Version 1.2.1 for the DAGR Actuator
//###########################################################################

// The firmware is separated into 6 tabs (a - f), make sure all files are stored in the folder 
// named a_DA024F121_ACT.
...
const float maxPowersourceCurrent = 3.4;       //Very rough approximation of max current draw from the power source [Amps]
...
const float alignStrength = 1.8;    // Percentage of power used to calibrate the sensor on start-up
...
//#######_GRAVITY COMPENSATION_#########
bool gravityCompMode = true;
int currentQthreshold = 5;      //minimum current that defines a force applied on the actuator's output [A]
float positionThreshold = 0.1; 
float kgc = 0.001;                //Proportional gain of the gravity compensation controller
float holdPosition = 0;         //moving position depending on external forces applied to the actuator's output
